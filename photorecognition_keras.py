...
def main():
    ...
    model.summary() #prints out a summary of each layer 
    model = CompileNetwork(model)
...
def PrepNeuralNetwork():
    ...
    model = keras.models.Sequential([
        ...
        #convolution and pooling layers to make it extract more broad features
        keras.layers.Conv2D(32, (3,3), activation="relu", input_shape=(height, width, 3)),
        keras.layers.MaxPool2D(),
        keras.layers.Conv2D(64, (3,3), activation="relu", input_shape=(height, width, 3)),
        keras.layers.MaxPool2D(),
    ...
    """_summary_
    Normalizes the values by removing the mean and scaling the data (heard this was better for unsupervised algorithms)
    returns a scaled version of the features extracted earlier 
    """
def Normalize(features):
    ...
    """_summary_
    uses K Means clustering to group the data into unsupervised clusters so each cluster can be manually labelled.
    """
def Cluster(features):
    
    kmeans = KMeans(n_clusters=outputs, random_state=0)
